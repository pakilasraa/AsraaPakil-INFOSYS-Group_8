...
class PosController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'items.*.temperature' => ['nullable', 'string', 'in:hot,iced'], // ✅ Hot/Iced
        ...
        DB::transaction(function () use ($validated) {
            ...
            foreach ($validated['items'] as $line) {
                ...
                $size = $line['size'] ?? null;
                $temperature = $line['temperature'] ?? null; // 🔥 kunin temperature per line
                ...
                // i-store lahat ng kailangan sa lineItems (kasama temp)
                $lineItems[] = [
                    'product'     => $product,
                    'qty'         => $qty,
                    'price'       => $price,
            ...
            // Save items to transaction_items (kasama na ang temperature)
            foreach ($lineItems as $li) {
                TransactionItem::create([
                    ...
                    'quantity'       => $li['qty'],
                    'price'          => $li['price'],
                    'temperature'    => $li['temperature'], // ✅ save Hot/Iced
